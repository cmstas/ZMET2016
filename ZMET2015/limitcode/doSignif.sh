	...
	if [ $MASS1 -gt $MASS2 ]; then
		...
		if [ -e ${INDIR}datacard_mGluino_${MASS1}_mLSP_${MASS2}_.txt ]; then
			if [ "$doSignificance" = true ]; then
				echo "Running command: combine -M ProfileLikelihood --uncapped 1 --significance -n mGluino${MASS1}_mLSP${MASS2}_ --rMin \"-5\" ${INDIR}datacard_mGluino_${MASS1}_mLSP_${MASS2}_.txt > log/significance_mGluino${MASS1}_mLSP${MASS2}.txt 2>&1"
				combine -M ProfileLikelihood --uncapped 1 --significance -n mGluino${MASS1}_mLSP${MASS2}_ --rMin "-5" ${INDIR}datacard_mGluino_${MASS1}_mLSP_${MASS2}_.txt > log/significance_mGluino${MASS1}_mLSP${MASS2}.txt 2>&1
			else
